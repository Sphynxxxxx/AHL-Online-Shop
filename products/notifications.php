...
// Query to fetch notifications and associated order details
$sql = "
    SELECT n.*, o.reference_number, p.product_name, o.status, n.order_id
    FROM notifications n
    LEFT JOIN orders o ON n.order_id = o.id
    LEFT JOIN order_details od ON o.id = od.order_id 
    LEFT JOIN products p ON od.product_id = p.product_id  
    WHERE n.order_id IN (SELECT id FROM orders WHERE customer_id = ?)
    ...
    <div class="notifications-container">
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                $order_status = ucfirst(str_replace('_', ' ', htmlspecialchars($row['status'])));
        ...
        ?>
                <div class="notification">
                    ...
                    <?php if ($row['status'] === 'ready_to_pick_up') { ?>
